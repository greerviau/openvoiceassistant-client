    ...
    def check_for_updates(self):
        if not self.updating:
            if self.current_branch not in UPDATE_BRANCHES:
                logger.warning("You are not on an update branch, skipping update check")
                return
                ...
                if local_commit != remote_commit:
                    ...
                    latest_version = (
                        subprocess.check_output(
                            ["git", "show", f"origin/{self.current_branch}:VERSION"]
                        )
                        .decode("utf-8")
                        .strip()
                    )
